...
export class ProductosComponent implements OnInit {
  ...
  ordenarPrecio() {
    if (this.ordenarForm.value.precio === 'Menor a mayor precio') {
      ...
      let auxProducto: any;
      auxProducto = this.productos;
      for (let i = 0; i < auxProducto.length - 1; i ++) {
        let Auxprecio1: any;
        Auxprecio1 = auxProducto[i].precio.replace('$', '');
        Auxprecio1 = Auxprecio1.replace(',', '');
        let Auxprecio2: any;
        Auxprecio2 = auxProducto[i + 1].precio.replace('$', '');
        Auxprecio2 = Auxprecio2.replace(',', '');
        console.log('i', i, 'p1', auxProducto[i], 'p', +Auxprecio1);
        console.log('i+1', i + 1, 'p2', auxProducto[i + 1], 'p2', +Auxprecio2);
        if (+Auxprecio1 > +Auxprecio2) {
          let aux: any;
          aux = auxProducto[i];
          auxProducto[i] = auxProducto[i + 1];
          auxProducto[i + 1] = aux;
        }
      }
