...
export class FiltrosComponent implements OnInit {
  ...
  verificarFiltro(producto, idSubnivel, disponible, precio1, precio2, stock, index) {
    ...
    console.log(precio1 + 0);
    if (disponible === 'Ambos') {
      if ((producto.subnivelId === idSubnivel) && (Auxprecio > precio1 + 0) && (Auxprecio < precio2 + 0) &&
      ...
      if ((producto.subnivelId === idSubnivel) && (producto.disponible === estado) && (Auxprecio > precio1 + 0) &&
        (Auxprecio < precio2 + 0) && (producto.cantidad >= stock)) {
          this.productos[index].filtro = true;
      } else {
        this.productos[index].filtro = false;
      }
