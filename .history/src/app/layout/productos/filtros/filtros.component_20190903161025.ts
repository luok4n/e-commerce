...
export class FiltrosComponent implements OnInit {
  ...
  verificarFiltro(producto, idSubnivel, disponible, precio1, precio2, stock, index) {
    ...
    Auxprecio = Auxprecio + 0;
    if (disponible === '') {
      if ((producto.idSubnivel === idSubnivel) && (producto.precio > precio1) && (producto.precio < precio2) &&
      ...
      console.log('precio', Auxprecio);
      if ((producto.idSubnivel === idSubnivel) && (producto.disponible === estado) && (producto.precio > precio1) &&
        (producto.precio < precio2) && (producto.cantidad >= 10000)) {
          this.productos[index].filtro = true;
      } else {
        this.productos[index].filtro = false;
      }
